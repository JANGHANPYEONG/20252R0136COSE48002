...
이 스크립트는 RGB 이미지를 입력으로 받아 분류와 회귀 헤드를 동시에 학습하는 2D CNN
파이프라인입니다. JSON 설정 파일로 모델 구조, 데이터셋 위치, 증강 파라미터, 세그멘테이션
옵션, 최적화 하이퍼파라미터를 정의하면 해당 정보를 기반으로 전체 학습·평가 루프가
자동으로 구성됩니다. CUDA 전용 환경을 가정하고 있으며 재현성보다 성능을 우선하도록
cudnn 설정을 튜닝했습니다.

주요 기능:
  - CLI 인자로 설정 파일, 체크포인트 저장 주기, MLflow 사용 여부 제어
  - Albumentations 기반 데이터 증강/전처리와 train/val/test 로더 자동 생성
  - 세그멘테이션 마스크(사전 계산 혹은 온더플라이) 적용 방식 선택 가능
  - 모델 설정 검증, 파라미터 개수/클래스 수 출력, pos_weight 계산
  - MLflow 로깅(선택)으로 하이퍼파라미터, 학습 곡선, 메트릭, 체크포인트 기록
