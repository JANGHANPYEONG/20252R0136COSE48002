...
class MLflowLogger:
    ...
    def start_run(self, run_name: Optional[str] = None):
        """MLflow run을 시작합니다."""
        if run_name is None:
            run_name = f"hsi_vit_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        else:
    ...
    def log_metrics(self, metrics: Dict[str, float], step: Optional[int] = None):
        ...
        for key, value in metrics.items():
            if isinstance(value, (int, float)) and not (np.isnan(value) or np.isinf(value)):
                ...
                print(f"Skipping invalid metric {key}: {value}")

        if filtered_metrics:
            mlflow.log_metrics(filtered_metrics, step=step)
            if step is not None:
                print(f"Step {step} - Logged metrics: {list(filtered_metrics.keys())}")
            else:
                print(f"Logged metrics: {list(filtered_metrics.keys())}")
        else:
            print("No valid metrics to log")
    ...
    def log_training_curve(self, train_losses: list, val_losses: list, 
        ...
        if plot_keys is None:
            plot_keys = ["r2", "mse", "auc", "combined_score"]
