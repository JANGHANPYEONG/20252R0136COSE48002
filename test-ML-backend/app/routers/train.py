...
def run_train_task(self, config: Dict):
    ...
    try:
        ...
        self.update_state(state='TRAINING', meta={
            ...
            'process_pid': process.pid,  # subprocess PID 사용
        ...
        try:
            ...
            # stdout에서 MLflow run ID 추출 (학습 스크립트가 마지막 줄에 run_id 출력한다고 가정)
            mlflow_run_id = None
            if stdout:
                lines = stdout.strip().split('\n')
                for line in reversed(lines):
                    if line.strip() and not line.startswith('[') and len(line.strip()) > 10:
                        mlflow_run_id = line.strip()
                        break
...
async def cancel_train(train_id: str):
    ...
    try:
        ...
        # 2. PID를 통해 학습 프로세스만 정상 종료 시도 (subprocess이므로 Worker와 분리됨)
        process_pid = None
        if result.info and result.info.get('process_pid'):
            process_pid = result.info.get('process_pid')
            
            try:
                # subprocess이므로 안전하게 SIGTERM 전송
                ...
                try:
                    ...
                    print(f"Training subprocess {process_pid} still running after SIGTERM")
                    # 필요시 SIGKILL로 강제 종료 (subprocess이므로 Worker에 영향 없음)
                    os.kill(process_pid, signal.SIGKILL)
                    print(f"Sent SIGKILL to training subprocess {process_pid}")
        ...
        # 메시지 구성
        if process_pid:
            message = "Training cancellation requested. The training subprocess will be terminated safely without affecting the worker."
        else:
