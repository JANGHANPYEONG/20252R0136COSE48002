...
// 서버에 직접 파일 업로드 API (FormData 사용)
export const addFileUploadDirect = async (
  file, // 업로드할 파일
  meatId, // 이력번호
  userId, // 사용자 ID
  ...
  try {
    ...
    // 3. 서버에 파일 업로드
    const response = await fetch(`http://${apiIP}/meat/add/file-upload-direct`, {
      method: 'POST',
...
export default addFileUploadDirect;
